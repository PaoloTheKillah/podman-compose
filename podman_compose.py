...
def mount_dict_vol_to_bind(compose, mount_dict):
    ...
    try:
        ...
        out = compose.podman.output(["volume", "inspect", vol_name])
    print("Volume inspect output: %r", out)
    src = json.loads(out.decode(encoding="utf-8"))[0]["Mountpoint"]
...
class PodmanCompose:
    ...
    def run(self):
        ...
        if podman_path != "podman":
            if os.path.isfile(podman_path) and os.access(podman_path, os.X_OK):
                ...
                if dry_run == False:
                    raise IOError("Binary {} has not been found.".format(podman_path))
        ...
        for service_name, service_desc in services.items():
            ...
            for num in range(1, replicas + 1):
                name0 = "{project_name}_{service_name}_{num}".format(
                    project_name=project_name, service_name=service_name, num=num
                )
                if num == 1:
    ...
    def _init_global_parser(self, parser):
        ...
        parser.add_argument(
            ...
            choices=["1pod", "1podfw", "hostnet", "cntnet", "publishall", "identity"],
            default="1podfw",
        )
